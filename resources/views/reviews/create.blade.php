...
<body class="reviews-form-body">
    ...
    @include('partials.header', [
        'title' => 'Nieuwe Review',
        'subtitle' => 'Laat jouw ervaring achter',
    ...
    <div class="reviews-form-container">
        ...
        <form action="{{ route('reviews.store') }}" method="POST" class="reviews-form">
            @csrf
            <div>
                <label>Klantnaam:</label>
                <input type="text" name="klantnaam" value="{{ old('klantnaam') }}" required>
            ...
            <div>
                ...
                <select name="beoordeling" required>
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        <option value="{{ $i }}" {{ old('beoordeling') == $i ? 'selected' : '' }}>{{ $i }}</option>
                    @endfor
                </select>
            ...
            <div>
                ...
                <textarea name="opmerking" rows="3" required>{{ old('opmerking') }}</textarea>
                @error('opmerking')<div class="reviews-error">{{ $message }}</div>@enderror
            </div>

            <button type="submit" class="reviews-btn-submit">Opslaan</button>
