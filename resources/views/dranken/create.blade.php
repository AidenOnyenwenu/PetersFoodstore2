...
<body class="dranken-form-body">
    ...
    @include('partials.header', [
        'title' => 'Nieuwe Drank',
        'subtitle' => 'Voeg een nieuwe drank toe aan het assortiment',
    ...
    <div class="dranken-form-container">
        ...
        <form action="{{ route('dranken.store') }}" method="POST" enctype="multipart/form-data" class="dranken-form">
            @csrf
            <div>
                <label>Naam:</label>
            ...
            <div>
                <label>Prijs (€):</label>
                <input type="number" step="0.01" name="prijs" value="{{ old('prijs') }}" required>
                @error('prijs')<div class="dranken-error">{{ $message }}</div>@enderror
            </div>

            <div>
                <label>Omschrijving:</label>
                <textarea name="omschrijving" rows="3">{{ old('omschrijving') }}</textarea>
            ...
            <button type="submit" class="dranken-btn-submit">Opslaan</button>
